...
/* The above code is using JavaScript to query a user by their ID and then excluding the 'password'
field from the returned user object. This is typically done for security reasons to prevent
sensitive information like passwords from being exposed in the response. */
    const user = await User.findById(userId).select('-password');
    if (!user) {
      return res.status(404).json({ message: 'User not found' });
    }
